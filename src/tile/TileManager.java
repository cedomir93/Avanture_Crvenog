...
//		loadMap("/maps/dungeon02.txt", 4);
	...
	public void getTileImage() {
		for (int i = 0; i < this.fileNames.size(); i++) {
			...
			if (((String)this.collisionStatus.get(i)).equals("true")) {
				collision = true;
			} else {
				collision = false;
			} 
	...
	public void draw(Graphics2D g2) {
		...
		if (this.drawPath) {
			...
			for (int i = 0; i < this.gp.pFinder.pathList.size(); i++) {
				this.gp.getClass();
				int worldX = ((Node)this.gp.pFinder.pathList.get(i)).col * 48;
				this.gp.getClass();
				int worldY = ((Node)this.gp.pFinder.pathList.get(i)).row * 48;
				int screenX = worldX - this.gp.player.worldX + this.gp.player.screenX;
				int screenY = worldY - this.gp.player.worldY + this.gp.player.screenY;
				this.gp.getClass();
				this.gp.getClass();
