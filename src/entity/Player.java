...
//		direction = "down";
	...
	public void setDialogue() {
		dialouge[0][0] = "You are level " + this.level + " now!\n" + 
			"You feel stronger!";
	}
...
//		invenotry.add(new OBJ_Lantern(gp));
	...
	public int getCurrentWeaponSlot() {
		...
		for (int i = 0; i < invenotry.size(); i++) {
			if (invenotry.get(i) == currentWeapon)
				currentWeaponSlot = i; 
		} 
	...
	public int getCurrentShieldSlot() {
		...
		for (int i = 0; i < invenotry.size(); i++) {
			if (invenotry.get(i) == currentShield)
				currentShieldSlot = i; 
		} 
...
//			gp.projectileList.add(projectile);
		...
		if (keyH.godModeOn == false) { //if statement ONLY for a DEBUG 
			if (life <= 0) {
...
//			damageProjectile(projectileList);
//			
//			
...
//					invenotry.add(gp.obj[gp.currentMap][i]);
					gp.playSE(1);
...
//	}
	...
	public void checkLevelUp() {
		...
		if (exp >= nextLevelExp) {
			...
			dialouge[0][0] = "You are level " + level + " now! \nYou feel stronger"; 
	...
	public void selectItem() {
		...
		if (itemIndex < invenotry.size()) {
			
			Entity selectedItem = invenotry.get(itemIndex);
	...
	public int searchItemInInventory(String itemName) {
		...
		for (int i = 0; i < invenotry.size(); i++) {
			if (invenotry.get(i).name.equals(itemName)) {
				itemIndex = i;
				break;
			}
		}
	...
	public boolean canObtainItem(Entity item) {
		...
		if (item.stackable == true) {
			...
			if (index != 999) {
				invenotry.get(index).amount++;
				canObtain = true;
			}
			else { // New item
				if (invenotry.size() != maxInventorySize) {
					invenotry.add(item);
					canObtain = true;
				}
			}
...
//		g2.drawString("Invinsible: " + invincibleCounter, 10, 40);
	}
}
