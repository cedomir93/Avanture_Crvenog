...
//		gp.player.life = 3;
	...
	public void drawMessage() {
		...
		for (int i = 0; i < message.size(); i++) {
			...
			if (message.get(i) != null) {
				...
				int counter = messageCounter.get(i) + 1; // messagCounter ++
	...
	public void drawDialougeScreen() {
		...
		if (npc.dialouge[npc.dialogueSet][npc.dialogueIndex] != null) {
//			currentDialouge = npc.dialouge[npc.dialogueSet][npc.dialogueIndex];
			
			char characters[] = npc.dialouge[npc.dialogueSet][npc.dialogueIndex].toCharArray();
	...
	public void drawInventory(Entity entity, boolean cursor) {
		...
		// DRAW PLAYER'S ITEMS
		for (int i = 0; i < entity.invenotry.size(); i++) {
			
			// EQUIPT CURSOR
			if (entity.invenotry.get(i) == entity.currentWeapon || entity.invenotry.get(i) == entity.currentShield || entity.invenotry.get(i) == entity.currentLight) {
				g2.setColor(new Color(240, 190, 90));
				g2.fillRoundRect(slotX, slotY, gp.tileSize, gp.tileSize, 10, 10);
			}
			
			g2.drawImage(entity.invenotry.get(i).down1, slotX, slotY, null);
			
			// DISPLAY THE AMOUNT
			if (entity == gp.player && entity.invenotry.get(i).amount > 1) {
				
				g2.setFont(g2.getFont().deriveFont(32f));
				int amountX;
				int amountY;
				
				String s = "" + entity.invenotry.get(i).amount;
		...
		if (cursor == true) {
			...
			int curstorHeight = gp.tileSize;
		
			// DRAW CUSROR
			g2.setColor(Color.white);
			g2.setStroke(new BasicStroke(3));
			g2.drawRoundRect(cursorX, cursorY, cursorWidth, curstorHeight, 10, 10);
			...
			if (itemIndex < entity.invenotry.size()) {
			
				drawSubWindow(dFrameX, dFrameY, dFrameWidth, dFrameHeight);
			
				for (String line: entity.invenotry.get(itemIndex).desription.split("\n")) {
	...
	public void trade_buy() {
		...
		if (itemIndex < npc.invenotry.size()) {
			...
			int price = npc.invenotry.get(itemIndex).price;
			String text = "" + price;
			...
			// BUY AN ITEM
			if (gp.keyH.enterPressed == true) {
				if (npc.invenotry.get(itemIndex).price > gp.player.coin) {
					subState = 0;
					npc.startDialogue(npc, 2);
				}
				else {
					if (gp.player.canObtainItem(npc.invenotry.get(itemIndex)) == true) {
						gp.player.coin -= npc.invenotry.get(itemIndex).price;
					}
					else {
						subState = 0;
	...
	public void trade_sell(){
		...
		if (itemIndex < gp.player.invenotry.size()) {
			...
			int price = gp.player.invenotry.get(itemIndex).price / 2;
			String text = "" + price;
			...
			if (gp.keyH.enterPressed == true) {
				...
				if (gp.player.invenotry.get(itemIndex) == gp.player.currentWeapon || gp.player.invenotry.get(itemIndex) == gp.player.currentShield) {
					commandNum = 0;
					subState = 0;
					npc.startDialogue(npc, 4);
				}
				else {
					if (gp.player.invenotry.get(itemIndex).amount > 1) {
						gp.player.invenotry.get(itemIndex).amount--;
					}
					else {
						gp.player.invenotry.remove(itemIndex);
					}
